...
use super::data_provider::{DocumentDataProvider, DataProviderError};
use super::model::{DocumentMongo, DocumentResponse, CreateDocumentRequest, UpdateDocumentRequest};
...
impl DocumentService {
    ...
    pub async fn create_document(&self, request: CreateDocumentRequest) -> Result<DocumentResponse, DataProviderError> {
        ...
        let created_doc = self.data_provider.find_document_by_id(&doc_id).await?
    ...
    pub async fn get_document_by_id(&self, doc_id: &str) -> Result<Option<DocumentResponse>, DataProviderError> {
        let mongo_doc = self.data_provider.find_document_by_id(doc_id).await?;
        Ok(mongo_doc.map(|doc| doc.to_response()))
    }
    
    pub async fn update_document(&self, doc_id: &str, request: UpdateDocumentRequest) -> Result<Option<DocumentResponse>, DataProviderError> {
        let updated = self.data_provider.update_document(doc_id, request.content).await?;
